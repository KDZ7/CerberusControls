...
namespace hidraw
{
    ...
    int Hidraw::write(void *buf, size_t len)
    {
        ...
        int res = hid_write(device, (unsigned char *)buf, len);
    ...
    int Hidraw::read(void *buf, size_t len)
    {
        ...
        int res = hid_read(device, (unsigned char *)buf, len);
        if (res < 0)
        {
    ...
    int Hidraw::readTimeout(void *buf, size_t len, int timeout)
    {
        ...
        int res = hid_read_timeout(device, (unsigned char *)buf, len, timeout);
        if (res < 0)
        {
            LOG_ERROR("Failed to read data");
            return iio::STATUS::ERROR;
