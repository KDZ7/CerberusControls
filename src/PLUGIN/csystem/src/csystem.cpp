...
namespace csystem
{
    controller_interface::CallbackReturn Csystem::on_init()
    {
        ...
        topic_pub_ = auto_declare<std::string>("topic_pub", "joint_states");
        topic_sub_ = auto_declare<std::string>("topic_sub", "joint_commands");
        ...
        movetime_ = auto_declare<double>("movetime", 25.0);
        ...
        test_mode_ = auto_declare<bool>("test_mode", false);
    ...
    controller_interface::CallbackReturn Csystem::on_configure([[maybe_unused]] const rclcpp_lifecycle::State &previous_state)
    {
        ...
        rt_joint_position_buffers_.writeFromNonRT(std::vector<double>(num_joints, 0.0));
        rt_joint_velocity_buffers_.writeFromNonRT(std::vector<double>(num_joints, 0.0));
        rt_joint_effort_buffers_.writeFromNonRT(std::vector<double>(num_joints, 0.0));
        rt_movetime_buffer_.writeFromNonRT(movetime_);
        ...
        rt_joint_state_pub_->msg_.position.resize(num_joints, 0.0);
        rt_joint_state_pub_->msg_.velocity.resize(num_joints, 0.0);
        rt_joint_state_pub_->msg_.effort.resize(num_joints, 0.0);
        joint_command_sub_ = this->get_node()->create_subscription<sensor_msgs::msg::JointState>(topic_sub_, rclcpp::QoS(10), std::bind(&Csystem::command_callback, this, std::placeholders::_1));
        rt_movetime_pub_ = std::make_unique<realtime_tools::RealtimePublisher<std_msgs::msg::Float64>>(this->get_node()->create_publisher<std_msgs::msg::Float64>("movetime", rclcpp::QoS(10)));
        rt_movetime_pub_->msg_.data = movetime_;
        movetime_sub_ = this->get_node()->create_subscription<std_msgs::msg::Float64>(
            "movetime",
            rclcpp::QoS(10),
            [this](const std_msgs::msg::Float64::SharedPtr msg)
            {
                ...
                {
                    LOG_WARN("Received an invalid movetime (NaN/Inf)");
                    return;
                }
                LOG_INFO("Received movetime command: " << msg->data);
                rt_movetime_buffer_.writeFromNonRT(msg->data);
            });
        ...
        previous_errors_.resize(num_joints, 0.0);
        pid_controllers_.resize(num_joints, control_toolbox::Pid(kp_, ki_, kd_, i_clamp_, -i_clamp_));
    ...
    controller_interface::CallbackReturn Csystem::on_activate([[maybe_unused]] const rclcpp_lifecycle::State &previous_state)
    {
        ...
        for (auto &interface : state_interfaces_)
        {
            ...
            else if (interface.get_interface_name() == "movetime/time")
                movetime_state_interfaces_ = &interface;
        }
        ...
        for (auto &interface : command_interfaces_)
        {
            if (interface.get_interface_name() == hardware_interface::HW_IF_POSITION)
                ...
                movetime_command_interfaces_ = &interface;
        }
        LOG_INFO("Cystem details: " << std::endl
                                    ...
                                    << "> Movetime: " << movetime_ << std::endl
                                    << "> Movetime State Interface: " << (movetime_state_interfaces_ ? "Found" : "Not found") << std::endl
                                    << "> Movetime Command Interface: " << (movetime_command_interfaces_ ? "Found" : "Not found") << std::endl
    ...
    controller_interface::CallbackReturn Csystem::on_cleanup([[maybe_unused]] const rclcpp_lifecycle::State &previous_state)
    {
        ...
        rt_movetime_buffer_.reset();
        position_state_interfaces_.clear();
        velocity_state_interfaces_.clear();
        effort_state_interfaces_.clear();
        movetime_state_interfaces_ = nullptr;
        position_command_interfaces_.clear();
        velocity_command_interfaces_.clear();
        effort_command_interfaces_.clear();
        movetime_command_interfaces_ = nullptr;
    ...
    controller_interface::InterfaceConfiguration Csystem::state_interface_configuration() const
    {
        ...
        if (this->get_node()->has_parameter("test_mode"))
            this->get_node()->get_parameter("test_mode", test_mode);
        if (!test_mode)
            config.names.push_back("movetime/time");
    ...
    controller_interface::return_type Csystem::update([[maybe_unused]] const rclcpp::Time &time, [[maybe_unused]] const rclcpp::Duration &period)
    {
        ...
        {
            if (i < position_command_interfaces_.size())
            {
                double current_position = position_state_interfaces_[i].get().get_value();
                double target_position = positions[i] + joint_offsets_[i];
        ...
        if (rt_joint_state_pub_ && rt_joint_state_pub_->trylock())
        {
            ...
            for (size_t i = 0; i < joints_.size(); ++i)
            {
                if (i < position_state_interfaces_.size())
                    msg.position[i] = position_state_interfaces_[i].get().get_value();
                if (i < velocity_state_interfaces_.size())
                    msg.velocity[i] = velocity_state_interfaces_[i].get().get_value();
                if (i < effort_state_interfaces_.size())
                    msg.effort[i] = effort_state_interfaces_[i].get().get_value();
            }
            rt_joint_state_pub_->unlockAndPublish();
        }

        if (movetime_command_interfaces_)
        {
            double movetime_value = *rt_movetime_buffer_.readFromRT();
            if (!std::isnan(movetime_value) && !std::isinf(movetime_value))
            {
                [[maybe_unused]] bool _ = movetime_command_interfaces_->set_value(movetime_value);
            }
        }
        if (rt_movetime_pub_ && rt_movetime_pub_->trylock())
        {
            rt_movetime_pub_->msg_.data = *rt_movetime_buffer_.readFromRT();
            rt_movetime_pub_->unlockAndPublish();
        }
        return controller_interface::return_type::OK;
    }
    void Csystem::command_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
        ...
        for (size_t i = 0; i < msg->name.size(); ++i)
        {
            ...
            if (it != joints_.end())
            {
                ...
                {
                    double offset = (idx < joint_offsets_.size()) ? joint_offsets_[idx] : 0.0;
                    double target_position = msg->position[i] + offset;
                    if (std::isnan(target_position) || std::isinf(target_position))
                    {
                        LOG_WARN("Received an invalid target position (NaN/Inf) for joint [" << msg->name[i] << "]");
                        continue;
                    }
                    double current_position = position_state_interfaces_[idx].get().get_value();
                    if (std::abs(target_position - current_position) > dthreshold_)
