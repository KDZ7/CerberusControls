...
namespace hwsystem
{
  ...
  private:
    ...
    std::unordered_map<std::string, double> joint_max_positions_;
    std::unordered_map<std::string, double> joint_min_positions_;
    std::unordered_map<std::string, double> joint_min_velocities_;
    std::unordered_map<std::string, double> joint_max_velocities_;

    std::unordered_map<std::string, double> last_joint_command_positions_;

    std::string config_file_path;
    std::unordered_map<std::string, uint8_t> joint_idx_map;
    std::shared_ptr<idevice::Servo> servo;
  };
